                ...
                <tbody>
                  {filtered.map(e => (
                    <tr key={e.id} style={{ cursor: 'pointer' }} onClick={() => nav('/equipos/' + e.id)}>
                      ...
                      <td>{e.sistema || '-'}</td>
                      <td onClick={ev => ev.stopPropagation()}>{(() => { const ac = e.cred_url || e.ip_host || e.subgrupo; return ac ? <span className="cred-cell mono"><a href={/^https?:\/\//.test(ac) ? ac : 'http://' + ac} target="_blank" rel="noreferrer" className="cred-url">{ac}</a><CopyBtn text={ac} tip="Copiar URL / acceso" /></span> : <span className="subtle">-</span>; })()}</td>
